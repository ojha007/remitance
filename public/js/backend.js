...
__webpack_require__(/*! ./bootstrap-datepicker.min */ "./Resources/assets/js/bootstrap-datepicker.min.js");

__webpack_require__(/*! ./modal-validation */ "./Resources/assets/js/modal-validation.js");
...
/***/ "./Resources/assets/js/modal-validation.js":
/*!*************************************************!*\
  !*** ./Resources/assets/js/modal-validation.js ***!
  \*************************************************/
/*! no static exports found */
...
$("document").ready(function () {
  ...
  $("form.bootstrap-modal-form").on("submit", function (submission) {
    submission.preventDefault(); // Set vars.

    var form = $(this),
        url = form.attr("action"),
        submit = form.find("[type=submit]"); // Check for file inputs.
    ...
    $.ajax({
      ...
      processData: false // Responses.

    }).always(function (response) {
      // Reset errors.
      resetModalFormErrors();
      var modal = $(".bootstrap-modal-form");
      ...
      if (response.status === 422) {
        var errors = $.parseJSON(response.responseText).errors; // Iterate through errors object.

        $.each(errors, function (field, message) {
          // $(".bootstrap-modal-form").find($.attr("autofocus")).focus();
          //handle arrays
          if (field.indexOf('.') != -1) {
            field = field.replace('.', '['); //handle multi dimensional array
          ...
          if (fieldElement.length > 0) {
            ...
            if (fieldElement.attr('type') === 'hidden') {
              fieldElement.parent().append('<div class="col-md-12 col-md-offset-2">' + '<p class="help-block">' + message + '</p>' + '</div>');
            } else {
              if (fieldElement.parent().hasClass('input-group')) fieldElement.parent('.input-group').parent().append('<p class="help-block text-danger">' + message + '</p>');else fieldElement.parent().append('<p class="help-block text-danger">' + message + '</p>');
            }
          } else {
            var fieldElement = form.find('label[for=' + field + ']');
            var formGroup = fieldElement.closest('.form-group');
            if (fieldElement.siblings().hasClass('input-group')) fieldElement.siblings('.input-group').parent().append('<p class="help-block text-danger">' + message + '</p>');else fieldElement.siblings().append('<p class="help-block text-danger">' + message + '</p>');
          }
          ...
          if (currentForm.length > 0) {
            var a = $(".bootstrap-modal-form").find(".has-error:first").find("input");
